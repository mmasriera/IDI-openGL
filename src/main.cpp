...
GLfloat ambient0[] = {0.1, 0.1, 0.1, 1.0},
		difussa0[] = {1.0, 1.0, 0.0, 1.0},
		especular012[] = {0.9, 0.7, 0.7, 1.0},
		ambient12[] = {0.05, 0.05, 0.05, 1.0},
		difusa12[] = {1.0, 1.0, 1.0, 1.0};
...
/**
*recorre tot el vector de vertex, tenint en compte que x,y i z estan en ordre
*/
...
void escalaModel(double escalaX, double escalaY, double escalaZ) {
    // perque estigui proporcionat es devideix 1/mida_maxima
...
/**
* inicialitza la camara ortogonal
*/
void initOrtogonal() {
...
void kbrd(unsigned char key, int x, int y) {
    ...
    if( key == 'h' ) {
        ...
        if(! modeEuler) 
        	cout << " no es pot girar amb angles d'euler en mode primera persona " << endl;
        else 
        ...
        if(modeEuler) { //el meu mde euler es el seu primera perosna
            ...
            if( ortogonal == false)
            	cout << " P E R S P E C T I V A" << endl;
